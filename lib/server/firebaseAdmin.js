...
const GOOGLE_AUDIENCE =
  ...
  `//iam.googleapis.com/${GOOGLE_WORKLOAD_IDENTITY_PROVIDER}`;
...
async function initializeAdmin() {
  ...
  try {
    ...
    // Create a GoogleAuth client with our external account credentials
    const auth = new GoogleAuth({
      credentials,
      scopes: ["https://www.googleapis.com/auth/cloud-platform"],
    });

    // ✅ Construct an explicit credential wrapper for Firebase Admin
    const firebaseCredential = {
      getAccessToken: async () => {
        const tokenResponse = await auth.getAccessToken();
        if (!tokenResponse?.token) {
          throw new Error("Failed to obtain STS token from Workload Identity Federation.");
        }
        console.log("🔑 WIF token obtained successfully.");
        return {
